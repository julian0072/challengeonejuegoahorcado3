...
const lista =
  localStorage.getItem("listaPalabras") === null
    ? listaPalabras.join(",")
    : localStorage.getItem("listaPalabras");
...
function agregarPalabraLista(array = "") {
  ...
  mapear.map((palabra, posicion) => {
    agregar_palabra.querySelector(
      ".lista_palabras ul"
    ).innerHTML += `<li class="display-inline-block" id="palabra_${posicion}">${palabra}</li>`;
  });
}
agregarPalabraLista();

const itemArray = [].slice.call(
  agregar_palabra.querySelector(".lista_palabras ul").children
);
...
function fnObtenerListaCreada() {
  let getList = [];
  const obtenerLista = [].slice.call(
    agregar_palabra.querySelectorAll(".lista_palabras ul li")
  );
    ...
    switch (bloque) {
      ...
      case "desistir":
        if (bloque === "iniciar") {
          ...
          const tecladoMovil = [].slice.call(
            document.querySelectorAll(".teclas input")
          );
...
function fnPalabraSecreta() {
  ...
  WordSecret.split("").map(
    (letra, posicion) =>
      (document.querySelector(".palabras").innerHTML +=
        '<input type="text" id="p' + posicion + '" class="line"/>')
  );
  secreto = WordSecret;
  return WordSecret;
}
